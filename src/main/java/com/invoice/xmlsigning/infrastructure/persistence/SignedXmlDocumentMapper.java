...
public interface SignedXmlDocumentMapper {
    ...
    @Mapping(target = "id", expression = "java(new com.invoice.xmlsigning.domain.model.SignedXmlDocumentId(entity.getId()))")
    @Mapping(source = "invoiceId", target = "invoiceId")
    @Mapping(source = "invoiceNumber", target = "invoiceNumber")
    @Mapping(source = "originalXml", target = "originalXml")
    @Mapping(source = "signedXml", target = "signedXml")
